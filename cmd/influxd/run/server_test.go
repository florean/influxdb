...
func TestServer_Query_IntoTarget_Sparse(t *testing.T) {
	...
	test.addQueries([]*Query{
		&Query{
			...
			// 6 points "written" because some null results were included in that time range.
			exp: `{"results":[{"series":[{"name":"result","columns":["time","written"],"values":[["1970-01-01T00:00:00Z",6]]}]}]}`,
		},
		&Query{
			name:    "confirm results",
			params:  url.Values{"db": []string{"db0"}},
			command: `SELECT * FROM baz`,
			// But the "null" results are not included when retrieving the results.
